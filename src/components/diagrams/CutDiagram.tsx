...
import { msg, type Locale } from "@/lib/i18n/config";
import {
  DIAGRAM_CAPTION_MSGS,
  DIAGRAM_LABEL_MSGS,
  type DiagramVariant,
} from "@/lib/i18n/messages/diagrams";
...
/** English captions (legacy en-only surface; locale-aware callers pass `locale` to CutDiagram). */
export const DIAGRAM_CAPTIONS: Record<DiagramVariant, string> = Object.fromEntries(
  (Object.keys(DIAGRAM_CAPTION_MSGS) as DiagramVariant[]).map((v) => [
    v,
    DIAGRAM_CAPTION_MSGS[v].en,
  ]),
...
const label = (v: DiagramVariant, l: Locale) => msg(DIAGRAM_LABEL_MSGS[v], l);

const ART: Record<DiagramVariant, (l: Locale) => JSX.Element> = {
  batons: (l) => (
    <g>
      ...
      <Label x={132} y={110}>{label("batons", l)}</Label>
    </g>
  ),
  "bite-size": (l) => (
    <g>
      ...
      <Label x={143} y={76}>{label("bite-size", l)}</Label>
    </g>
  ),
  "quarter-lengthwise": (l) => (
    <g>
      ...
      <Label x={112} y={110}>{label("quarter-lengthwise", l)}</Label>
    </g>
  ),
  "thin-spread": (l) => (
    <g>
      ...
      <Label x={28} y={110}>{label("thin-spread", l)}</Label>
    </g>
  ),
  strips: (l) => (
    <g>
      ...
      <Label x={34} y={110}>{label("strips", l)}</Label>
    </g>
  ),
  mash: (l) => (
    <g>
      ...
      <path
        ...
        strokeLinecap="round"
      />
      <Label x={52} y={110}>{label("mash", l)}</Label>
    </g>
  ),
  shred: (l) => (
    <g fill="none" stroke={INK} strokeWidth="2.5" strokeLinecap="round">
      ...
      <Label x={48} y={110}>{label("shred", l)}</Label>
    </g>
  ),
  ribbons: (l) => (
    <g fill="none" strokeLinecap="round">
      ...
      <Label x={46} y={110}>{label("ribbons", l)}</Label>
    </g>
  ),
  "wedge-handle": (l) => (
    <g>
      ...
      <Label x={118} y={96}>{label("wedge-handle", l)}</Label>
    </g>
  ),
};
...
export function CutDiagram({
  ...
  const caption = msg(DIAGRAM_CAPTION_MSGS[variant], locale);
  return (
